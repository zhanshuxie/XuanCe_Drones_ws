"""
高层规划器训练脚本 — MAPPO on SimpleSpreadDrones

在纯 2D 粒子环境中训练 3-agent 合作覆盖任务（simple_spread）。
训练完成后可与低层 Navigate PPO 模型拼接部署（见 demo_hierarchical_deploy.py）。

配置: xuance/configs/mappo/DroneSpread/SimpleSpreadDrones.yaml



用法:
    训练:       python demo_simple_spread_planner.py --mode train
    基准训练:   python demo_simple_spread_planner.py --mode benchmark
    可视化测试: python demo_simple_spread_planner.py --mode test --model-dir results/mappo/SimpleSpreadDrones/best_model/best_model.pth --device cpu --test-episode 1
"""
import os
import argparse
import numpy as np
...
def parse_args():
    parser = argparse.ArgumentParser("High-level Planner: MAPPO on SimpleSpreadDrones")
    parser.add_argument("--algo", type=str, default="mappo")
    parser.add_argument("--env", type=str, default="DroneSpread")
    parser.add_argument("--env-id", type=str, default="SimpleSpreadDrones")
